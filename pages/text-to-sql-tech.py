        ...
        if submitted2:
            if len(st.session_state.dataframe_query) > 0:
                with st.spinner('Generating response'):
                    ...
                    #         output = agent.invoke(
                    #             st.session_state.dataframe_query + ". If any charts or graphs or plots were created save them localy and include the save file names in your response. Remember to set pandas option to pd.set_option('display.float_format', str). You must use all rows of the dataframe.",
                    #             verbose=False).get('output')
                    #
